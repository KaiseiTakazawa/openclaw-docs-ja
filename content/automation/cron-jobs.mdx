...
channel formatting.
\* Heartbeat-only responses (`HEARTBEAT\_OK` with no real content) are not delivered.
...
Telegram supports forum topics via `message\_thread\_id`. For cron delivery, you can encode
...
{
"name": "Morning brief",
"schedule": { "kind": "cron", "expr": "0 7 \* \* \*", "tz": "America/Los\_Angeles" },
...
\* Both apply to `cron/runs/.jsonl` files.
Webhook behavior:
...
\* `cron.runLog.maxBytes`: `2\_000\_000` bytes
\* `cron.runLog.keepLines`: `2000`
### How it works
\* Isolated runs create session entries (`...:cron::run:`) and transcript files.
...
\* After each run append, `cron/runs/.jsonl` is size-checked:
...
\* many noisy recurring jobs writing to the same `cron/runs/.jsonl`
What to do:
...
## Troubleshooting
### “Nothing runs”
\* Check cron is enabled: `cron.enabled` and `OPENCLAW\_SKIP\_CRON`.
