...
\* \*\*Smart suppression\*\*: If nothing needs attention, the agent replies `HEARTBEAT\_OK` and no message is delivered.
...
\* If the tool returns `needs\_approval`, you resume with a `resumeToken` and `approve` flag.
...
| Output | Delivered if not `HEARTBEAT\_OK` | Heartbeat prompt + event | Announce summary (default) |
