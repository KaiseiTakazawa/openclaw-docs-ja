...
Token resolution is account-aware. Config token values win over env fallback. `DISCORD\_BOT\_TOKEN` is only used for the default account.
...
```
If you only set `DISCORD\_BOT\_TOKEN` and do not create a `channels.discord` block, runtime fallback is `groupPolicy="allowlist"` (with a warning in logs), even if `channels.defaults.groupPolicy` is `open`.
...
\* `spawnSubagentSessions` must be true to auto-create/bind threads for `sessions\_spawn({ thread: true })`.
\* `spawnAcpSessions` must be true to auto-create/bind threads for ACP (`/acp spawn ... --thread ...` or `sessions\_spawn({ runtime: "acp", thread: true })`).
...
pluralkit: {
enabled: true,
token: "pk\_live\_...", // optional; needed for private systems
},
},
},
}
...
\* Treat bot tokens as secrets (`DISCORD\_BOT\_TOKEN` preferred in supervised environments).
